...
/* Биометрический вход (WebAuthn/Passkeys). Если файла нет — кнопки биометрии
   просто не работают, вход по номеру телефона никак не затрагивается. */
if (is_file(__DIR__ . '/webauthn.php')) require_once __DIR__ . '/webauthn.php';
...
/* ==== Биометрический вход (WebAuthn/Passkeys) — доступно без входа ==== */
if (($preAction === 'bio_login_options' || $preAction === 'bio_login_finish') && function_exists('wa_handle_preauth')) {
    header('Content-Type: application/json');
    header('Cache-Control: no-store');
    if (!rate_ok('bio', 30, 60)) { echo json_encode(['error' => 'too_many']); exit; }
    wa_handle_preauth($pdo, $preAction);
    exit;
}
...
if ($action) {
    ...
    /* ==== Биометрический вход (WebAuthn/Passkeys): управление ключами ==== */
    if (in_array($action, ['bio_status', 'bio_reg_options', 'bio_reg_finish', 'bio_disable'], true) && function_exists('wa_handle_authed')) {
        wa_handle_authed($pdo, $action, $ME);
        exit;
    }
    if ($action === 'face_has') {
