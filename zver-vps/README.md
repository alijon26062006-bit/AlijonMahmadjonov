...
## Перенос базы со старого хостинга

Балансы, заказы и история переносятся дампом. Порядок: сначала установка,
потом импорт — скрипт `import-db.sh` делает всё остальное сам.

**1. Снять дамп на старом хостинге**

```bash
mysqldump --default-character-set=utf8mb4 --single-transaction \
  -u СТАРЫЙ_ЮЗЕР -p СТАРАЯ_БАЗА > zver_dump.sql
```

Если хостинг шаред и SSH нет — phpMyAdmin → вкладка «Экспорт» → формат SQL,
кодировка **utf8mb4**.

**2. Перекинуть на новый сервер**

```bash
scp zver_dump.sql root@IP_СЕРВЕРА:/root/
```

**3. Импортировать**

```bash
curl -fsSL https://raw.githubusercontent.com/alijon26062006-bit/AlijonMahmadjonov/refs/heads/claude/zver-taj-server-deploy-gom4he/zver-vps/import-db.sh -o imp.sh && sudo bash imp.sh /root/zver_dump.sql
```

Скрипт сам: прочитает доступы из `config.php`, сделает резервную копию
текущей базы, зальёт дамп, дополнит структуру до версии 13.1 через
`?setup=1` и покажет количество строк и общий баланс до и после.

Если что-то пойдёт не так — база не потеряется, скрипт напечатает команду
отката на резервную копию.

> **Важно про кодировку.** Дамп в `latin1` превратит кириллицу и таджикские
> буквы в «кракозябры», причём необратимо. Всегда выгружайте в `utf8mb4`.

> **Важно про webhook.** У одного бот-токена может быть только один webhook.
> Как только новый сервер его заберёт, старая копия бота перестанет получать
> сообщения. Переносите базу в момент, когда заказов нет, иначе то, что
> пройдёт на старом сервере после снятия дампа, потеряется.
