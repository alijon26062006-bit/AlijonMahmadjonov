...
case "\${1:-help}" in
    ...
    api)
        ...
        if [ -z "\${2:-}" ]; then
            echo "Использование: stars-bot api https://ваш-адрес [ПОРТ]"
        ...
        APIPORT="\${3:-8443}"
        # Слушаем только изнутри: снаружи в бота ходят через nginx с
        # шифрованием. На 0.0.0.0 остался бы открытый порт без HTTPS.
        sudo -u "\$RUN_USER" "\$APP/.venv/bin/python" "\$APP/setup.py" --set \
