"""Кабинет разработчика: ключи, баланс, заказы, вебхук, статистика.

Личный кабинет живёт в самом боте, а не на отдельном сайте — и это не
упрощение, а единственная честная схема для этого проекта. Клиент бота
уже опознан Telegram, у него уже есть баланс и история заказов. Сайт
означал бы второй вход (логин с паролем), второй список людей и второй
баланс, который разошёлся бы с первым в первый же день.

Документация при этом лежит по адресу самого API: разработчик открывает
ссылку из кабинета и сразу видит примеры на своём языке.
"""
