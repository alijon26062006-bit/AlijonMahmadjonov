...
_RAW["DEPOSIT_REQUISITES"] = (
    ...
    "{holder}{bank}"
    "{extra}"
    "<blockquote>[[warn]] Сумма — <b>ровно {amount}</b>.\n"
    "Копейки не убирайте: по ним бот узнаёт ваш перевод.\n"
    "{dc_block}</blockquote>\n\n"
...
_RAW["DEPOSIT_WAITING"] = (
    "[[wait]] <b>Ждём перевод</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Как только банк сообщит о переводе на <b>{amount}</b>, баланс "
    "пополнится <b>сам</b>. Обычно это меньше минуты.\n\n"
    "<blockquote>[[ok]] Чек присылать не нужно: копейки в сумме — "
    "это и есть метка вашего платежа.\n\n"
    "Если через пару минут ничего не изменилось, пришлите скриншот "
    "чека — разберёмся вручную.</blockquote>"
)
...
#: поэтому живёт отдельно от оформления: поменять текст экрана можно, а
#: название способа в старых заявках менять нельзя.
RU_METHOD = "Из России (Сбербанк/Тинькофф)"

_RAW["DEPOSIT_RU_HOW"] = (
    "🇷🇺 <b>Оплата из России</b>\n"
    f"<code>{LINE}</code>\n\n"
    "<b>1.</b> Откройте Сбербанк, Тинькофф или другой ваш банк\n"
    "<b>2.</b> Выберите <b>Переводы за рубеж</b> (международный перевод)\n"
    "<b>3.</b> Найдите получателя <b>«Душанбе Сити»</b>, страна — "
    "Таджикистан\n"
    "<b>4.</b> Введите номер получателя:\n\n"
    "<code>{number}</code>\n\n"
    "<b>5.</b> Укажите, сколько отправляете <b>в рублях</b> — любую сумму\n"
    "<b>6.</b> Переведите и <b>сохраните чек</b>\n\n"
    "<blockquote>[[warn]] Банк сам пересчитает рубли в сомони. В чеке "
    "появится строка <b>«Зачислено получателю»</b>, и рядом с ней — "
    "сумма в TJS с копейками.\n\n"
    "Именно её вы назовёте боту на следующем шаге. Эти копейки — ваш "
    "опознавательный знак: такой суммы прямо сейчас больше ни у кого "
    "нет, и по ней бот найдёт ваш перевод сам.</blockquote>\n\n"
    "<i>Перевели — нажмите «Я отправил»</i>"
)

_RAW["DEPOSIT_RU_ASK"] = (
    "🇷🇺 <b>Сумма из чека</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Откройте чек и найдите строку <b>«Зачислено получателю»</b>.\n"
    "Там написана сумма <b>в сомони (TJS)</b> — не в рублях.\n\n"
    "<blockquote>[[warn]] Перепишите её <b>точно, вместе с копейками</b>: "
    "<code>10.08</code>, а не <code>10</code>.\n\n"
    "Копейки — единственное, чем ваш перевод отличается от чужих. "
    "Без них бот не сможет его найти, и зачисление будет ждать "
    "владельца.</blockquote>\n\n"
    "[[search]] <i>Введите сумму из чека:</i>"
)

_RAW["DEPOSIT_RU_ROUND"] = (
    "[[warn]] <b>Сумма без копеек</b>\n\n"
    "В чеке почти всегда есть копейки — посмотрите ещё раз. Если там "
    "правда ровная сумма, пришлите её снова, и я приму: просто такой "
    "перевод дольше искать.\n\n"
    "<i>Второй раз то же число — приму как есть.</i>"
)

_RAW["DEPOSIT_RU_WAIT"] = (
    "🕔 <b>Ждём ваш перевод</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Сумма записана: <b>{amount}</b>\n\n"
    "<blockquote>Деньги из России идут от нескольких минут до суток — "
    "это зависит от банка, не от нас. Как придут, баланс пополнится "
    "сам, и я напишу.</blockquote>\n\n"
    "<i>Пришлите чек — с ним разбор быстрее, если что-то пойдёт не так.</i>"
)

_RAW["DEPOSIT_ALREADY"] = (
    "[[warn]] <b>У вас уже есть заявка</b>\n"
    f"<code>{LINE}</code>\n\n"
    "├ Сумма: <b>{amount}</b>\n"
    "├ Способ: {method}\n"
    "└ Создана: {when}\n\n"
    "<blockquote>Вторая заявка сейчас только помешает. Бот узнаёт ваш "
    "перевод по сумме с копейками — если заявок будет две, подойдут обе, "
    "и зачисление придётся ждать от владельца.\n\n"
    "Оплатите эту заявку или отмените её — и заводите новую.</blockquote>"
)

_RAW["DEPOSIT_RECEIPT_TWICE"] = (
    "[[ok]] <b>Чек уже получен</b>\n\n"
    "По заявке на <b>{amount}</b> он у нас есть — присылать ещё раз не "
    "нужно.\n\n"
    "<blockquote>Как деньги придут, баланс пополнится сам, и я напишу. "
    "Повторные чеки зачисление не ускоряют.</blockquote>"
)

_RAW["DEPOSIT_RECEIPT_OLD"] = (
    "[[fail]] <b>Этот чек уже присылали</b>\n\n"
    "Он относится к прошлой заявке. За один перевод баланс пополняется "
    "один раз.\n\n"
    "<blockquote>Если вы сделали новый перевод — пришлите чек именно "
    "от него.</blockquote>"
)

_RAW["DEPOSIT_CANCELLED"] = (
    "[[ok]] <b>Заявка отменена</b>\n\n"
    "Теперь можно завести новую.\n\n"
    "<blockquote>Если деньги вы всё-таки отправили — напишите в "
    "поддержку {support}, владелец зачислит их руками.</blockquote>"
)
...
_RAW["SPONSOR_ASK"] = (
    "[[warn]] <b>Остался один шаг</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Подпишитесь на наш канал — и возвращайтесь, всё откроется.\n\n"
    "<blockquote>Там выходят скидки, розыгрыши и новости о новых "
    "играх.</blockquote>"
)
_RAW["SPONSOR_NOT_YET"] = (
    "[[fail]] Пока не вижу подписки. Подпишитесь и нажмите ещё раз."
)
_RAW["SPONSOR_OK"] = "[[party]] Спасибо! Доступ открыт."
