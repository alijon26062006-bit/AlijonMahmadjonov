...
_RAW["DEPOSIT_ASK_RECEIPT"] = (
    "[[wait]] <b>Ждём поступление</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Как только банк сообщит о переводе на <b>{amount}</b>, баланс "
    "пополнится <b>сам</b>. Обычно это занимает меньше минуты.\n\n"
    "<blockquote>Если через пару минут баланс не изменился — пришлите "
    "сюда скриншот чека, и мы зачислим вручную.\n\n"
    "Чаще всего причина одна: сумма отличается на копейку.</blockquote>"
)
