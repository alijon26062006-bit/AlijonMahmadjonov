...
_RAW["DEPOSIT_ASK_RECEIPT"] = (
    ...
    "\n\nЧек нужен только <b>в первый раз</b>: по нему мы запомним ваш "
    "счёт. Дальше пополнения будут приходить сразу, без чека."
    "</blockquote>"
)

#: Постоянному клиенту чек уже не нужен: его счёт мы знаем.
_RAW["DEPOSIT_WAIT_KNOWN"] = (
    "[[wait]] <b>Ждём перевод</b>\n"
    f"<code>{LINE}</code>\n\n"
    "Как только банк сообщит о переводе на <b>{amount}</b>, баланс "
    "пополнится <b>сам</b>. Обычно это меньше минуты.\n\n"
    "<blockquote>[[ok]] Ваш счёт нам знаком по прошлым переводам — "
    "чек присылать не нужно.\n\n"
    "Если через пару минут баланс не изменился, пришлите скриншот "
    "чека: скорее всего сумма разошлась на копейку.</blockquote>"
)
