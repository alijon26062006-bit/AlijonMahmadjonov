...
_RAW["DEPOSIT_REQUISITES"] = (
    ...
    "{dc_block}</blockquote>\n\n"
    "<i>Оплатили — нажмите кнопку ниже</i> [[point]]"
)
...
_RAW["DEPOSIT_ASK_RECEIPT"] = (
    "📸 <b>Пришлите скриншот чека</b>\n"
    f"<code>{LINE}</code>\n\n"
    "<b>Отправьте сюда фото или скриншот чека — прямо в этот чат, "
    "одним сообщением.</b>\n\n"
    "<blockquote>Без чека заявку не проверить: по одному переводу мы не "
    "узнаем, чей он.\n\n"
    "Зачислим <b>{amount}</b> на баланс сразу после проверки — обычно "
    "это несколько минут.</blockquote>"
)

_RAW["DEPOSIT_NEED_PHOTO"] = (
    "📸 Пришлите <b>фото или скриншот</b> чека — по тексту оплату "
    "не проверить."
)
