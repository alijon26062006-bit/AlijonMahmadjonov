...
export default function CartPage({ cfg }: { cfg?: ShopConfig }) {
  ...
  useEffect(() => {
    ...
    setForm((f) => ({
      ...f,
      customer_name: f.customer_name || user.first_name || '',
  ...
  if (!user) {
    return <Empty icon="shopping-bag" title="Войдите, чтобы собрать корзину"
                  note="Вход занимает одно нажатие"
                  action={{ label: 'Войти', to: '/login' }} />;
  }
  if (done) return <OrderDone order={done} cfg={cfg} />;
  if (isLoading) return <div className="skeleton" style={{ height: 240, marginTop: 12 }} />;
  if (!cart || cart.items.length === 0) {
    return <Empty icon="shopping-bag" title="Корзина пуста"
  ...
  return (
    <div className="fade-in section">
      ...
      {cart.items.map((i) => (
        <div key={i.variant_id} className="cart-row">
          <Link to={`/p/${i.product_id}`} className="cart-photo">
            {i.photo
              ? <img src={photoUrl(i.photo)} alt="" />
          ...
          <div className="cart-info">
            ...
            <div className="cart-price">{fmtPrice(i.price)}</div>
          </div>
          <div className="cart-qty">
            ...
            <button className="cart-drop" onClick={() => drop.mutate(i.variant_id)}>
      ...
      <div className="total-box">
        <div className="total-row"><span>Товары</span>
          <span>{fmtPrice(cart.goods_total)}</span></div>
        <div className="total-row"><span>Доставка</span>
          <span>{shipping === 0 ? 'бесплатно' : fmtPrice(shipping)}</span></div>
        <div className="total-row total-final"><span>Итого</span>
      ...
      {step === 'cart' ? (
        ...
        <div className="section">
          <h2 className="section-title">Куда доставить</h2>
          ...
          <div className="field">
            ...
            <div className="chips">
              {[['courier', 'Курьером'], ['post', 'Почтой'],
                ...
                <button key={v} className={`chip ${form.delivery === v ? 'on' : ''}`}
            ...
            {form.delivery === 'pickup' && pickup && (
              <div className="field-hint">Забрать: {pickup}</div>
            )}
          </div>

          <div className="field">
            <span className="field-label">Имя <span style={{ color: 'var(--danger)' }}>*</span></span>
            <input value={form.customer_name}
                   onChange={(e) => setForm({ ...form, customer_name: e.target.value })} />
          </div>
          <div className="field">
            <span className="field-label">Телефон <span style={{ color: 'var(--danger)' }}>*</span></span>
            <input inputMode="tel" placeholder="+7 701 123 45 67" value={form.phone}
          ...
          {form.delivery !== 'pickup' && (
            <>
              <div className="field">
                <span className="field-label">Город <span style={{ color: 'var(--danger)' }}>*</span></span>
              ...
              <div className="field">
                <span className="field-label">Адрес <span style={{ color: 'var(--danger)' }}>*</span></span>
          ...
          <div className="field">
            ...
            <textarea rows={2} placeholder="Например: позвонить за час"
                      value={form.comment}
  ...
  return (
    <div className="fade-in section">
      <div className="done-box">
        <div className="done-icon"><Icon name="check" size={34} strokeWidth={2.4} /></div>
      ...
      {order.payment && (
        <div className="notice" style={{ marginTop: 14 }}>
          ...
          {order.payment.link && (
            <a className="btn btn-primary btn-block" style={{ marginTop: 10 }}
