...
if ! ./scripts/backup.sh; then
  echo >&2
  echo "Резервная копия не получилась — ничего не удалено." >&2
  echo "Удалять аккаунты без копии нельзя: вернуть их будет неоткуда." >&2
  echo "Разберитесь с сообщением выше и запустите снова." >&2
  exit 1
fi
