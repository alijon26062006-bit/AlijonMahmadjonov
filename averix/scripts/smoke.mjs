...
// Проверяется не размер комиссии, а то, что исполнителю сказали, сколько он
// получит, до отправки отклика. Сейчас площадка без комиссии, и текст об этом
// прямой; включат комиссию — текст назовёт её и остаток.
step('исполнителю сказали, сколько он получит, до отправки отклика',
  /(комиссия платформы|не берёт комиссию)/i.test(fee),
  (fee.match(/.{0,90}(комиссия платформы|не берёт комиссию).{0,60}/i) || [''])[0]);
await page.getByRole('button', { name: 'Далее' }).click();
