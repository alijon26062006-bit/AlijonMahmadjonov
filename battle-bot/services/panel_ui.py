...
def votes(
    ...
    text = (
        ...
        "⭐ Купленных на пару: <b>сколько угодно</b>\n\n"
...
def join_requests(waiting: int, approved: int, auto: bool, rows
    ...
    text = (
        ...
        f"{listing}{more}\n\n"
        "<blockquote>Список заявок Telegram боту не отдаёт: он видит только "
        "те, что пришли при нём. Зато принять человека бот может по одному "
        "его ID — поэтому любой, кто нажмёт в боте «Я подписался», "
        "впускается в канал сразу, даже если пришёл по чужой ссылке.\n\n"
        "Всё, что накопилось раньше, принимается в самом Telegram: канал → "
        "Заявки на вступление → «Принять всех».</blockquote>"
    )
