...
Две команды, и хостинг готов принимать клиентов:

```bash
git clone <ваш форк этого репозитория>
cd AlijonMahmadjonov
sudo bash hosting/install.sh    # ставит всё ПО и настраивает сервер
sudo bash hosting/setup.sh      # спрашивает домен, токен бота, пароль админа — и проверяет результат
```

Третья команда нужна, только если что-то пошло не так:

```bash
sudo bash hosting/scripts/doctor.sh   # проверяет весь хостинг и говорит, что именно чинить
```
...
Без внешних зависимостей (без PHPUnit) — свой минимальный runner. 53 теста:
