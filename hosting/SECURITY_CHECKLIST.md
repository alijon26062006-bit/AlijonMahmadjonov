...
## Файловый менеджер и Telegram-бот сайта (добавлено)

| Что | Статус | Как сделано |
|---|---|---|
| Выход за пределы каталога сайта (`../`, абсолютный путь, URL-кодирование, двойное кодирование) | PASS | `Path::normalize` + `realpath` + `Path::isInside` на каждом пути; тест `test_file_manager_blocks_escape_from_site_directory` |
| Симлинк наружу | PASS | `realpath` существующего пути сверяется с корнем сайта; тест `test_file_manager_blocks_symlink_escape` |
| Zip Slip и зип-бомбы | PASS | нормализация имён внутри архива + проверка числа записей и заявленного размера ДО распаковки |
| Проверка синтаксиса PHP перед сохранением | PASS | `PhpSyntax` через `token_get_all(TOKEN_PARSE)` — без запуска процессов и без временных файлов, поэтому нет и поверхности для shell-инъекции |
| Пути сервера в сообщениях об ошибках | PASS | `PhpSyntax::cleanMessage` вырезает `in /path on line N` |
| Токен бота в публичных файлах | PASS | токен только в `.env` над `public/`; тест проверяет, что в шаблоне webhook его нет |
| Токен бота в интерфейсе | PASS | показывается маска `123456789:AAEa************`, полный токен не возвращается |
| Токен бота в логах | PASS | `TelegramWebhook::scrub` вырезает токен из любого текста перед показом и записью |
| Секрет webhook | PASS | случайный, 32 символа, пересоздаётся при каждом подключении; в `webhook.php` сверяется через `hash_equals` |
| `.env` через браузер | PASS | лежит над `public/`; nginx отдельно запрещает `*.env` — проверено запросом: 403 |
| Скачивание `.env` из панели | PASS | требует явного подтверждения |
| Изоляция клиентов после смены прав | PASS | каталог сайта `2770 client:hosting-web` — другой клиент не заходит в него вообще; проверено на двух системных пользователях |
| Вызовы Telegram Bot API | PASS | только с сервера (`TelegramWebhook`), в браузер токен не попадает |
